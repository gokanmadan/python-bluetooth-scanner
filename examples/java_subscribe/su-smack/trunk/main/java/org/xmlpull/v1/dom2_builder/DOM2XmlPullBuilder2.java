...
public class DOM2XmlPullBuilder2 {

	private static final Log log = LogFactory.getLog(DOM2XmlPullBuilder2.class);
	
    public DOM2XmlPullBuilder2() { }

    protected synchronized Document newDoc() throws XmlPullParserException {
        try {
    ...
    static class BuildProcess {
    	...
    	private static final Log log = LogFactory.getLog(BuildProcess.class);
        ...
        private Element parseSubTree()
            throws XmlPullParserException, IOException
        {
        	//log.debug("parseSubTree using pp="+pp);
            ...
            */
            ...
            //log.debug("Returning element "+parent);
            return parent;
        }
            ...
            throws DOMException, XmlPullParserException
        {
            if(scanNamespaces) {
                ...
                for (int i = top; i >= pp.getNamespaceCount(0); --i)
                {
                    ...
                    for (int j = top; j > i; --j)
                    {
                    	String pfx = pp.getNamespacePrefix(j);
                        if(pfx != null && prefix != null && prefix.equals(pfx)) {
